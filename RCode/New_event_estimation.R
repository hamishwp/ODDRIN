...
PosteriorImpactPred <- function(ODDy, 
                                AlgoResultsFilename='HPC/mcmc_2025-02-27_162932.543295_MCMC_BDScore.05nocorr_LR40_M100_Npart1000NovAgg5_RandomFieldThree',#'HPC/abcsmc_2024-08-20_051627_alpha0.9_M60_Npart1000RealAgg5_propCOVmult0.2',
                                folder_write='IIDIPUS_Input_Alternatives/IIDIPUS_Input_NewEvents/'){
  ...
  ODDy_with_sampledfull <- sampleImpactODD(ODDy, AlgoResults, N_samples = 200)
  ...
  plot_GADM_impact_polygons(ODDy, polygon_list, gadm_level = 2, impact_type = 'mortality', summary = 'mean')
...
ODDRIN_vs_PAGER_hists <- function(ODDy, impact_type = 'mortality'){
  impact_type_samples = paste0(impact_type, '.s')
...
plot_predictive_panel <- function(ODDyAggWithImpact, sampled_full){
  ...
  p_mort_compare = ODDRIN_vs_PAGER_hists(ODDyAggWithImpact, impact='mortality')
  ...
  grid.arrange(grid.arrange(legendmort, ODDRIN_mortpred.05 + theme(legend.position="none"), ODDRIN_mortpred.50 + theme(legend.position="none"), 
