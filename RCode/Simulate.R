...
simulateDataSet <- function(nEvents, Omega, Model, dir, I0=4.5, cap=-300){
  ...
  for(i in 1:length(ODDpaths)){
    ...
    #account for measurement uncertainty by drawing from the relevant kernel
    #ODDSim@gmax$gmax <- round(rllaplace(1, log(ODDSim@gmax$gmax+1), 0.0333 * log(ODDSim@gmax$gmax+1)))
    #ODDSim@gmax$mortality <- round(rllaplace(1, log(ODDSim@gmax$mortality+1), 0.00333 * log(ODDSim@gmax$mortality+1)))
    #ODDSim@gmax$buildDestroyed <- round(rllaplace(1, log(ODDSim@gmax$buildDestroyed+1), 0.0167 * log(ODDSim@gmax$buildDestroyed+1)))
...
main_simulated <- function(){
  ...
  Omega_MAP = output$PhysicalValues %>% 
                  relist(skeleton=Model$skeleton) %>% unlist() %>% 
                  Proposed2Physical(Model)
  
  # Save the output
  saveRDS(object = output,
          file = paste0(dir,"IIDIPUS_Results/output_",
                        DateTimeString(),"_10events_init2_algo2.Rdata"))
...
#Omega_MAP <- output$OptimisationOut[18,3:NCOL(output$OptimisationOut)] %>% relist(skeleton=Model$skeleton) %>% unlist() %>% Proposed2Physical(Model)
...
logTarget(dir = dir,Model = Model, proposed = output$OptimisationOut[Omega,3:NCOL(output$OptimisationOut)] %>% relist(skeleton=Model$skeleton) %>% unlist() %>% Proposed2Physical(Model),AlgoParams = AlgoParams)
